...
# Фаза D: Gemini — продажи, доли, взаиморасчёт с Кариной (план, 15.09.2026; не начата)

Решения владельца: людям деньги не платим — партнёру доля в WWC$, Виктору рубли; расчёт с Кариной **день в день**; платит Карине только Виктор (партнёры платят Виктору); цены пока черновые, меняются командой без кода.

**Тариф (черновой):** `gemini_6m` — розница 3 990 ₽, опт 2 490 ₽, Виктору 1 000, партнёру 500 (= 5 WWC$), при прямой продаже Виктору 1 500. `gemini_18m` — те же числа.

## D1. Данные (миграция v10, additive)
- `service_tariffs(tenant_id, offer_code, retail_minor, wholesale_minor, owner_share_minor, partner_share_minor, direct_owner_share_minor, updated_by, updated_at)` — правится командой `тариф gemini_6m 3990 2490 1000 500 1500`.
- `service_sales(sale_id, tenant_id, ticket_id → support_tickets, offer_code, client_actor_id, partner_ref (из partner_referral_attributions, null = прямая), retail_minor, wholesale_minor, owner_share_minor, partner_share_minor, paid_to ('owner'), paid_currency (RUB|WUSD), paid_at, settled_with_admin_at, activated_until, status (paid|settled|activated|closed), created_by)`.
- Бонус партнёру — существующий `partner_bonus_ledger`, `entry_type = 'service_commission'`, `product_code = 'gemini'`, idempotency по `sale_id`.

## D2. Бот: в теме заявки (Карина и Виктор)
- Кнопка **«Оплачено»** → выбор тарифа (6/18) → бот считает по тарифу, пишет в тему: «Продажа #S-7: 3 990 ₽ · партнёр olga-samtsova +5 WWC$ · Виктору 1 000 ₽ · **Карине 2 490 ₽ — перевести сегодня**». Кнопка **«Переведено Карине»** → `settled_with_admin_at`. Партнёру пуш «+5 WWC$ за Gemini клиента #S-7», баланс.
- Кнопка **«Активировано до …»** (бот предлагает дату = сегодня + 6/18 мес) → `activated_until`, клиенту «лицензия активна до …», заявка закрывается как сейчас. Напоминание за 7 дней клиенту и в тему (как у клуба).
- Владелец: `оплата` через `сервис` — как раньше, только вызов из темы.

## D3. Группа Карины: две служебные темы
- Тема **«Бонусы»** (бот создаёт при `/forum`, id хранится в `support_forums`): каждое начисление — одной строкой «15.09 · olga-samtsova · +5 WWC$ · #S-7 · Gemini 6 мес». Ничего личного о клиенте.
- Тема **«Отчёты»**: команды в ней (и в личке Виктору): `отчёт` — с начала месяца: продаж, сумма, Карине переведено/не переведено (хвосты), по партнёрам — сколько начислено; `отчёт партнёр olga-samtsova`; `хвосты` — незакрытые переводы Карине. Ответы без имён клиентов.
- Ежедневно 21:00 МСК, если есть незакрытые переводы, — сообщение в «Отчёты»: «Не переведено Карине: N продаж, X ₽».

## D4. Таблица
Вкладка `Referral_Bonuses` уже зеркало бонусов — строки `service_commission` пишутся туда же (Операция «Gemini», payment_id = sale_id). Отдельной вкладки не заводим.

## Порядок и оценка
D1 + «Оплачено/Переведено» + бонус партнёру — 1 день; «Активировано» + напоминания — 0,5; темы «Бонусы»/«Отчёты» + отчёты + ежедневная сводка — 0,5. Тесты: unit на расчёт долей и идемпотентность; Postgres-тест на миграцию v10. Выкат: staging → проверка Виктором в группе `WWC_Карина` (staging-бот должен быть админом той же группы или своей) → prod тем же SHA.

## Вопросы владельцу (не блокируют старт)
1. Партнёру 500 ₽ = 5 WWC$ — курс 1 WWC$ = 100 ₽ подтверждён.
2. Если клиент — сам партнёр WWC (купил себе), партнёрская доля ему же?
3. «Активировано» ставит Карина или Виктор?
