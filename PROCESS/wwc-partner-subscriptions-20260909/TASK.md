...
- В MVP разрешены только две валюты: `RUB` и `BYN`.
...
- `RUB` и `BYN` без учёта регистра, другие валюты отклоняются;
- RUB: положительное целое число;
- BYN: положительное число, допускаются точка или запятая и максимум два знака после разделителя;
...
7. RUB и BYN записываются в minor units без float.
...
```text
Виктор пишет боту: оплата @partner <amount> <RUB|BYN>
