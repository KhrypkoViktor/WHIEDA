...
## План прод-среды NSP (предложение NSP-агента, 17.09)

Принцип: **отдельный контейнер + отдельная БД**, как staging сейчас; ничего общего с прод-базой WHIEDA (изоляция данных, нулевой риск для WHIEDA-бота, никаких RLS-сюрпризов).

| Шаг | Что | Кто / когда |
|---|---|---|
| 1 | Прод-бот в @BotFather (например `@NSP_bot`), токен — только в `secrets/api.env` прод-контейнера | Максим создаёт, передаёт лично |
| 2 | Домен: `nsp.sysarch.pro` (наш) или домен Максима (CNAME на 185.252.232.93) | Виктор решает |
| 3 | `/opt/whieda-nsp-prod/`: env из шаблона staging (`PLATFORM_SUPPORT_ADMIN_TELEGRAM_ID`=аккаунт поддержки, `PLATFORM_TENANT_PRODUCT_MEDIA_BASE_URL=https://media.sysarch.pro/nsp-media`), БД `whieda_nsp_prod` в том же Postgres-контейнере, 31 миграция из manifest, nginx vhost + TLS, `nsp-stagingctl` → копия `nsp-prodctl` с другим BASE/NAME/PORT | NSP-агент, полдня после п.1-2 |
| 4 | Образ `master-<sha>` тот же, что прошёл golden на staging; пакет v4 → apply в прод-БД; binding `nsp-maxim-prod-bot`; webhook | NSP-агент |
| 5 | Тот же golden-прогон против прод-БД (in-process, без Telegram) — 261/265 или выше | NSP-агент |
| 6 | Юридика: договор с юрлицом Максима (ответственность за контент), текст согласия на первом экране (реквизиты: название, УНП, адрес, контакт) | Виктор + Максим, до открытия бота людям |
| 7 | Открытие: сначала Максим + Виктор (сутки), потом команда Максима | Виктор |

Что НЕ делаем на проде: `--publish` (валидатор Core отказывает намеренно), общая БД с WHIEDA, партнёрские цены в боте.
