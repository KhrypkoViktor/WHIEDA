-- Shared-staging base schema for the structured advisor data plane.
-- Additive only: tenant catalog rows remain isolated by client_id.
...
alter table advisor_structured_products add column if not exists retail_prices jsonb;
