...
EXPECTED_APPLY_COUNT = 41  # +telegram_consent_v1, +site_request_plans_v10 (19.09.2026), +lead_actor_channels_v11 (20.09.2026), +site_request_contacts_v12, +renewal_services_v13 (24.09.2026), +crm_v14, +academy_v1, +academy_shelf_v15 (25.09.2026), +support_site_forum_v16 (26.09.2026)  # +marketing_consent_v17 (26.09.2026)
...
def build_offline_plan() -> ReleaseReport:
    ...
    if APPLY_ORDER[-1] != OUTBOX_SQL:
        failed.append("durable outbox SQL is not last in APPLY_ORDER")
    if APPLY_ORDER[-2] != PRICE_PLANE_SQL:
        failed.append("tenant release price-plane SQL must immediately precede durable outbox SQL")
    if APPLY_ORDER[-3] != RELEASE_PACKAGE_SQL:
        failed.append("tenant release package SQL must immediately precede price-plane SQL")
    if APPLY_ORDER[-4] != DATA_PLANE_SQL:
        failed.append("tenant advisor data-plane SQL must immediately precede release package SQL")
    if APPLY_ORDER[-5] != INBOX_SQL:
