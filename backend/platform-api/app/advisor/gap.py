...
GAP_TEXTS: dict[str, str] = {
    "unknown_product": (
        "Я пока не нашёл такой товар в текущем каталоге. "
        "Могу помочь с товаром, ценой, сравнением или собрать корзину. "
        "Как называется товар или что вы хотите решить?"
    ),
    "ambiguous_product": (
        "Нужно уточнить, о каком товаре речь — тогда смогу дать цену, карточку или фото."
    ),
    "unknown_followup": (
        "Подскажи, о каком товаре речь: название, фото, цена или сравнение?"
    ),
    "unsupported_topic": (
        "По этому запросу у меня нет готового сценария. "
        "Могу подсказать по товару, цене, сравнению, корзине, акциям или бизнес-вопросам WHIEDA."
    ),
    "missing_resource": (
    ...
    "medical_or_safety_boundary": (
        "Прибор и продукты WHIEDA не заменяют схему лечения диагноза. "
...
NEXT_STEPS: dict[str, list[str]] = {
    ...
    "unknown_followup": [
        "Назовите товар",
        "Спросите «цена …» с названием",
        "Попросите фото или сравнение",
    ],
    "unsupported_topic": [
