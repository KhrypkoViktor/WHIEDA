...
# One menu for «I did not understand at all»; every other gap says what is
# missing and what to do next (owner, 15–21.09.2026: «не кринжово»).
MENU_HINT = (
...
GAP_TEXTS: dict[str, str] = {
    "unknown_product": (
        "Такого товара в каталоге WHIEDA не нашёл. Уточните название — или откройте "
        "📦 Товары, покажу список и подскажу по каждому."
    ),
    "unrouted_message": (
        "Не понял вопрос. Напишите название товара или выберите направление — "
        "названия знать не обязательно.\n\n" + MENU_HINT
    ),
    "ambiguous_product": (
        "Нужно уточнить, о каком товаре речь — тогда смогу дать цену, карточку или фото."
    ),
    "unknown_followup": (
        "Чтобы ответить, нужен сам товар: назовите его или откройте 📦 Товары. "
        "Потом спросите цену, PV, фото или сертификат — отвечу по нему."
    ),
    "unsupported_topic": (
        "Такого сценария у меня нет — я про товары WHIEDA, цены, PV, применение и бизнес. "
        "Спросите про товар или выберите направление:\n\n" + MENU_HINT
    ),
    "missing_resource": (
...
ANSWER_MODE_BY_KIND: dict[str, str] = {
    # An unknown product is a clarification: the person names it, we answer.
    "unknown_product": "clarification",
