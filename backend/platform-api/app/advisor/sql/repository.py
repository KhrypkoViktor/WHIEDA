...
async def load_active_solution_bundles(conn, tenant_id: str) -> list[dict[str, Any]]:
    """Load only owner-published runtime bundles for the current tenant.

    ``to_jsonb`` deliberately keeps the reader compatible with the two bundle
    exports that existed before the Core route was introduced.
    """
