...
def _lead_detail_item(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "metadata": pick_lead_metadata(row.get("metadata")),
        # Согласия по заявке (кабинет партнёра, 26.09.2026): ПДн — версия и дата,
        # рассылка — отдельная галочка и дата, когда её поставили.
        "consent_version": row.get("consent_version"),
        "consent_at": _iso(row.get("consent_at")),
        "marketing_consent": bool(row.get("marketing_consent")),
        "marketing_consent_at": _iso(row.get("marketing_consent_at")),
        "created_at": _iso(row.get("created_at")),
        "updated_at": _iso(row.get("updated_at")),
        "meta": gap_meta("last_activity_at"),
    }
