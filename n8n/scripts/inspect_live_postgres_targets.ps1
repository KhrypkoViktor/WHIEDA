...
$local = Join-Path $env:TEMP ('inspect-live-postgres-targets-' + [guid]::NewGuid().ToString() + '.sh')
...
try {
  Invoke-RemoteProcess -FilePath 'scp.exe' -Password $SshPassword -Arguments @(
    ...
    '-o','BatchMode=no',
    $local,
    "${RemoteUser}@${RemoteHost}:/tmp/inspect_live_postgres_targets.sh"
  ) | Out-Null
  ...
  Invoke-RemoteProcess -FilePath 'ssh.exe' -Password $SshPassword -Arguments @(
    ...
    'bash /tmp/inspect_live_postgres_targets.sh'
  ) | Write-Output
}
finally {
