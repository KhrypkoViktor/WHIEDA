...
while [ $SECONDS -lt $END ]; do
  ...
  docker compose exec -T n8n sh -lc 'command -v ss >/dev/null 2>&1 && ss -tpn | grep 5432 || true'
  echo "---"
  sleep 1
done
'@
...
$local = Join-Path $env:TEMP ('inspect-container-pg-connections-' + [guid]::NewGuid().ToString() + '.sh')
...
try {
  Invoke-RemoteProcess -FilePath 'scp.exe' -Password $SshPassword -Arguments @(
    ...
    '-o','BatchMode=no',
    $local,
    "${RemoteUser}@${RemoteHost}:/tmp/inspect_container_pg_connections.sh"
  ) | Out-Null
  ...
  Invoke-RemoteProcess -FilePath 'ssh.exe' -Password $SshPassword -Arguments @(
    ...
    'bash /tmp/inspect_container_pg_connections.sh'
  ) | Write-Output
}
finally {
