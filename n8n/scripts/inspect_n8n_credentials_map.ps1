...
$local = Join-Path $env:TEMP ('inspect-n8n-credentials-map-' + [guid]::NewGuid().ToString() + '.sh')
...
try {
  Invoke-RemoteProcess -FilePath 'scp.exe' -Password $SshPassword -Arguments @(
    ...
    '-o','BatchMode=no',
    $local,
    "${RemoteUser}@${RemoteHost}:/tmp/inspect_n8n_credentials_map.sh"
  ) | Out-Null
  ...
  Invoke-RemoteProcess -FilePath 'ssh.exe' -Password $SshPassword -Arguments @(
    ...
    'bash /tmp/inspect_n8n_credentials_map.sh'
  ) | Write-Output
}
finally {
