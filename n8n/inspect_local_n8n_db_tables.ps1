...
where table_schema = 'public'
  and (
    table_name like 'advisor%'
    or table_name like 'content_%'
    or table_name like 'publish_%'
  )
order by table_name;
SQL
'@
...
$local = Join-Path $env:TEMP ('inspect-local-n8n-db-tables-' + [guid]::NewGuid().ToString() + '.sh')
...
try {
  Invoke-RemoteProcess -FilePath 'scp.exe' -Password $SshPassword -Arguments @(
    ...
    '-o','BatchMode=no',
    $local,
    "${RemoteUser}@${RemoteHost}:/tmp/inspect_local_n8n_db_tables.sh"
  ) | Out-Null
  ...
  Invoke-RemoteProcess -FilePath 'ssh.exe' -Password $SshPassword -Arguments @(
    ...
    'bash /tmp/inspect_local_n8n_db_tables.sh'
  ) | Write-Output
}
finally {
