"""Publish the local Dify response validator after a reviewed formatting fix."""
...
NODE_NAME = "Code: Validate Dify Response"
...
def main():
    ...
    code = (BASE_DIR / "validate_dify_response_current.js").read_text(encoding="utf-8")
    if "(^|\\n)\\s*Источник\\s*:" not in code:
        raise RuntimeError("Expected source-line duplicate guard is missing")
    ...
    node = next((item for item in workflow.get("nodes", []) if item.get("name") == NODE_NAME), None)
    if node is None:
        raise RuntimeError("Dify validator node not found")
    ...
    backup_path = backup_dir / f"advisor-whieda-phase1-before-dify-validator-polish-{datetime.now():%Y%m%d-%H%M%S}.json"
