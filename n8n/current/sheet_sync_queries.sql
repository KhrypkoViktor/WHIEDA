...
SELECT
  ...
  coalesce(trim_scale(round(bo.minor / 100.0, 2)), 0)                                        AS "Бонусы W$",
  ...
  trim_scale(coalesce(round((SELECT sum(l.amount_minor) FROM partner_payment_ledger l WHERE l.tenant_id = p.tenant_id AND l.ref_code = p.ref_code AND l.currency = 'WUSD') / 100.0, 2), 0)
...
SELECT
  ...
  trim_scale(round(l.amount_minor / 100.0, 2))                                   AS "Сумма",
  CASE l.currency WHEN 'RUB' THEN '₽' ELSE 'W' || chr(36) END                     AS "Вал.",  -- $ в литерале съедает узел n8n
  trim_scale(CASE l.currency WHEN 'RUB' THEN round(l.amount_minor / 10000.0, 2) ELSE round(l.amount_minor / 100.0, 2) END) AS "W$",
...
SELECT
  ...
  trim_scale(round(b.amount_minor / 100.0, 2))                                    AS "W$",
  ...
  trim_scale(round(sum(b.amount_minor) OVER (PARTITION BY b.actor_id ORDER BY b.created_at, b.entry_id) / 100.0, 2)) AS "Остаток W$",
