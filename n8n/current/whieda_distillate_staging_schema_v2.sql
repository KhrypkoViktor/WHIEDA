-- Isolated WHIEDA distillate staging. Runtime workflow never reads these tables.
CREATE EXTENSION IF NOT EXISTS pgcrypto;

CREATE TABLE IF NOT EXISTS advisor_distillate_import_runs (
  import_run_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id text NOT NULL,
  source_dir text NOT NULL,
  source_manifest jsonb NOT NULL DEFAULT '[]'::jsonb,
  validation_status text NOT NULL,
  staged_at timestamptz NOT NULL DEFAULT now(),
  completed_at timestamptz,
  report jsonb NOT NULL DEFAULT '{}'::jsonb,
  rollback_at timestamptz,
  rollback_reason text
);

CREATE TABLE IF NOT EXISTS advisor_distillate_records (
  staging_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  import_run_id uuid NOT NULL REFERENCES advisor_distillate_import_runs(import_run_id),
  tenant_id text NOT NULL,
  source_table text NOT NULL,
  external_record_id text NOT NULL,
  source_id text NOT NULL,
  source_locator text,
  raw_quote text,
  source_type text,
  authority_level text,
  extraction_pass text,
  source_hash text NOT NULL,
  content_hash text NOT NULL,
  payload jsonb NOT NULL,
  publication_status text NOT NULL DEFAULT 'blocked_raw',
  medical_review_state text NOT NULL DEFAULT 'not_required',
  business_review_state text NOT NULL DEFAULT 'not_required',
  owner_state text NOT NULL DEFAULT 'not_approved',
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (tenant_id, source_table, external_record_id, content_hash)
);

-- CSV lands here first. It lets every import be retried safely before rows
-- are merged into the immutable staging record store.
CREATE TABLE IF NOT EXISTS advisor_distillate_import_buffer (
  import_run_id uuid NOT NULL,
  tenant_id text NOT NULL,
  source_table text NOT NULL,
  external_record_id text NOT NULL,
  source_id text NOT NULL,
  source_locator text,
  raw_quote text,
  source_type text,
  authority_level text,
  extraction_pass text,
  source_hash text NOT NULL,
  content_hash text NOT NULL,
  payload jsonb NOT NULL,
  publication_status text NOT NULL,
  medical_review_state text NOT NULL,
  business_review_state text NOT NULL,
  owner_state text NOT NULL,
  loaded_at timestamptz NOT NULL DEFAULT now()
);

CREATE TABLE IF NOT EXISTS advisor_distillate_quarantine (
  quarantine_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  import_run_id uuid REFERENCES advisor_distillate_import_runs(import_run_id),
  tenant_id text NOT NULL,
  source_file text NOT NULL,
  source_row_number integer,
  source_id text,
  raw_row jsonb,
  reason text NOT NULL,
  resolution_status text NOT NULL DEFAULT 'unresolved',
  created_at timestamptz NOT NULL DEFAULT now()
);

CREATE TABLE IF NOT EXISTS advisor_distillate_links (
  link_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  import_run_id uuid NOT NULL REFERENCES advisor_distillate_import_runs(import_run_id),
  from_staging_id uuid NOT NULL REFERENCES advisor_distillate_records(staging_id),
  to_source_table text,
  to_external_record_id text,
  relation_type text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now()
);

CREATE TABLE IF NOT EXISTS advisor_distillate_review_queue (
  review_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  staging_id uuid NOT NULL REFERENCES advisor_distillate_records(staging_id),
  tenant_id text NOT NULL,
  review_type text NOT NULL,
  priority text NOT NULL DEFAULT 'medium',
  queue_status text NOT NULL DEFAULT 'pending',
  assigned_role text,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (staging_id, review_type)
);

CREATE TABLE IF NOT EXISTS advisor_distillate_candidates (
  candidate_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  import_run_id uuid NOT NULL REFERENCES advisor_distillate_import_runs(import_run_id),
  staging_id uuid NOT NULL REFERENCES advisor_distillate_records(staging_id),
  tenant_id text NOT NULL,
  candidate_type text NOT NULL,
  proposed_intent_id text,
  candidate_status text NOT NULL DEFAULT 'review_required',
  reason text,
  conflict_json jsonb NOT NULL DEFAULT '{}'::jsonb,
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (import_run_id, staging_id, candidate_type)
);

CREATE TABLE IF NOT EXISTS advisor_distillate_candidate_buffer (
  import_run_id uuid NOT NULL,
  staging_id uuid NOT NULL,
  tenant_id text NOT NULL,
  candidate_type text NOT NULL,
  proposed_intent_id text,
  candidate_status text NOT NULL,
  reason text,
  conflict_json jsonb NOT NULL
);

-- Derived clarifications are never mixed with observed conversations.
CREATE TABLE IF NOT EXISTS advisor_distillate_dialogue_observed (
  staging_id uuid PRIMARY KEY REFERENCES advisor_distillate_records(staging_id),
  import_run_id uuid NOT NULL REFERENCES advisor_distillate_import_runs(import_run_id),
  source_record_id text,
  provenance_verified boolean NOT NULL,
  follow_up text NOT NULL
);

CREATE TABLE IF NOT EXISTS advisor_distillate_dialogue_derived (
  staging_id uuid PRIMARY KEY REFERENCES advisor_distillate_records(staging_id),
  import_run_id uuid NOT NULL REFERENCES advisor_distillate_import_runs(import_run_id),
  source_record_id text,
  provenance_verified boolean NOT NULL,
  clarification_text text,
  publication_block_reason text NOT NULL
);

CREATE INDEX IF NOT EXISTS idx_distillate_records_tenant_table_status
  ON advisor_distillate_records (tenant_id, source_table, publication_status);
CREATE INDEX IF NOT EXISTS idx_distillate_candidates_run_status
  ON advisor_distillate_candidates (import_run_id, candidate_status);
-- Heal duplicate candidates left by an interrupted staging-only run before
-- enforcing the idempotency key. Runtime tables are never involved.
DELETE FROM advisor_distillate_candidates older
USING advisor_distillate_candidates newer
WHERE older.staging_id = newer.staging_id
  AND older.candidate_type = newer.candidate_type
  AND older.candidate_id < newer.candidate_id;
CREATE UNIQUE INDEX IF NOT EXISTS uq_distillate_candidates_staging_type
  ON advisor_distillate_candidates (staging_id, candidate_type);
