-- Руфия Тимотина, rufa.wwc.best (владелец, 19.09.2026): новый партнёр, взяла пакет
...
-- скажет, кто привёл, добавить строку (invitee 'rufa', inviter <owner_id>, 'admin_manual').
--
-- Запуск: python wwc_sql.py --file rufa_new_partner_2026-09-19.sql

BEGIN;

INSERT INTO lead_actors (actor_id, tenant_id, display_name, telegram_username, active)
VALUES ('rufa', 'whieda', 'Руфия Тимотина', 'rufinka7777', true)
...
VALUES ('rufa', 'whieda', 'rufa', 'named', 'RU', true, 1,
  '{"page_mode": "subdomain_site", "plan_code": "partner_subscription", "site_type": "subdomain_site",
    ...
    "display_name": "Руфия Тимотина", "leads_access": "partner", "owner_actor_id": "rufa",
    "public_site_url": "https://rufa.wwc.best/", "watcher_actor_id": "", "referrer_ref_code": ""}'::jsonb)
ON CONFLICT (ref_code) DO NOTHING;

INSERT INTO referral_invite_codes (tenant_id, invite_code, inviter_actor_id)
VALUES ('whieda', 'mTuoB8EM2988oGCt', 'rufa')
...
WHERE p.tenant_id = 'whieda' AND p.ref_code = 'rufa';
