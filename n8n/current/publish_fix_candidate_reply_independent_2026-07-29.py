"""Keep candidate reply independent from Google Sheets response payload."""
...
backup = base.parent / "backups" / f"advisor-whieda-before-candidate-reply-independence-{datetime.now():%Y%m%d-%H%M%S}.json"
backup.write_text(json.dumps(workflow, ensure_ascii=False, indent=2), encoding="utf-8")

# Google sync and access lookup are independent branches from the normalized candidate row.
