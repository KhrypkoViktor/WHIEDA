"""HLR HTTP target load, localhost guard, and check-target helpers."""
...
class HlrTargetError(Exception):
    pass


def validate_hlr_target_url(base_url: str) -> None:
    parsed = urlparse(base_url.rstrip("/"))
    if parsed.scheme == "https":
        raise HlrTargetError(f"HTTPS targets are forbidden for HLR HTTP lab: {base_url!r}")
    if parsed.scheme not in {"http"}:
        raise HlrTargetError(f"Unsupported target scheme: {parsed.scheme!r}")
    host = (parsed.hostname or "").lower()
    if not host:
        raise HlrTargetError(f"Invalid target URL: {base_url!r}")
    if host not in ALLOWED_HOSTS:
        raise HlrTargetError(
    ...
    for fragment in FORBIDDEN_HOST_FRAGMENTS:
        if fragment in lower:
            raise HlrTargetError(f"Forbidden production fragment in target: {fragment!r}")


def load_hlr_target(path: Path) -> dict[str, Any]:
    try:
        data = load_target(path)
    except TargetConfigError as exc:
        raise HlrTargetError(str(exc)) from exc
    validate_hlr_target_url(str(data["base_url"]))
    return data


def ensure_hlr_target(example: Path, local: Path) -> Path:
    if local.is_file():
        return local
    if not example.is_file():
        raise HlrTargetError(f"Missing HLR target example: {example}")
    local.write_text(example.read_text(encoding="utf-8"), encoding="utf-8")
    return local


def check_hlr_target(target_path: Path, *, timeout_seconds: float = 10.0) -> dict[str, Any]:
    target = load_hlr_target(target_path)
