#!/usr/bin/env python3
"""Build qa/telegram_golden/nsp_telegram_golden_cases_v1.jsonl from the NSP release package.
...
actually published. Re-run after every package rebuild.
    ...
    python qa/telegram_golden/build_nsp_golden_cases.py --package <dir>
"""
...
OUT = ROOT / "qa" / "telegram_golden" / "nsp_telegram_golden_cases_v1.jsonl"
TENANT = "nsp-maxim"
FORBIDDEN = [
    "Traceback",
    "Dify",
...
def build(package: Path) -> list[dict[str, Any]]:
    ...
    for sku, product in sorted(by_sku.items()):
        ...
        cases.append(
            _case(
                ...
                gap_kind=None,
                must_contain=[str(product["catalog_retail_usd"]), "$"],
                sets={"last_product_sku": sku},
                source_ref=ref,
            )
        )
    ...
    for faq in faqs:
        ...
        cases.append(
            _case(
                ...
                mode="structured_card",
                gap_kind=None,
                must_contain=[by_sku[sku]["canonical_name"]],
                sets={"last_product_sku": sku},
    ...
    for index, text in enumerate(UNKNOWN_PRODUCT_PROBES, start=1):
        cases.append(
            _case(
